...
func (h *hub) Start() {
	...
	for {
		...
		case postUpdate := <- h.UpdateClient:
			response := Response{UpdatedPostID: postUpdate.UpdatedPostID}
			if h.Connections[postUpdate.UserID] != nil {
				h.Connections[postUpdate.UserID].Send <- response
			}
